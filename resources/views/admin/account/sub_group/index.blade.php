                    ...
                    <table class="table datanew">
                        ...
                        @foreach($sub_groups as $subgroup)
                            <tr>
                                ...
                                <td>{{ $subgroup->name }}</td>
                                <td>{{ $subgroup->group_id ? $subgroup->group->name : '' }}</td>
                                <td>{{ Str::limit($subgroup->description, 150) }}</td>
                                <td>
                                    @can('update subgroup')
                                        <a class="me-3" href="{{route('sub_group.edit',$subgroup->id)}}">
                                            <img src="{{asset('/')}}admin/assets/img/icons/edit.svg" alt="img">
                                        </a>
                                    @endcan
                                    @can('delete subgroup')
                                        <form action="{{route('sub_group.destroy', $subgroup->id)}}" method="POST" class="sr-dl" >
                                            @csrf
                                            @method('delete')
